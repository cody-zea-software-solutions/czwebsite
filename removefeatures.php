...
if (isset($_POST['uid']) && isset($_POST['id']) && isset($_POST['tid'])) {
    ...
    if ($result->num_rows == 0) {
        ...
        Databases::IUD("DELETE FROM `user_has_features` WHERE `user_user_id` = '$uid' AND `features_f_id` = '$id'");
...
?>
            <div class="col-6  mt-4">
                <div class="card card-margine position-relative">
                    <!-- "+" Button at Top Right -->
                    <button class="btn btn-warning position-absolute plus-btn rounded-circle d-flex justify-content-center align-items-center" onclick="removeFeatures('<?php echo $_SESSION['user_id']; ?>','<?php echo $row['f_id']; ?>','<?php echo $tid; ?>');"><img src="assets/img/remove.svg" class="img-fluid ri"></button>
