...
if (isset($_POST['id']) && isset($_POST['catid'])) {
    ...
    if ($catid != 0) {
        $stmt = $conn->prepare("SELECT * FROM `blogs` INNER JOIN `blog_categories` ON `blogs`.`blog_categories_bc_id` = `blog_categories`.`bc_id` WHERE `bc_id` = ? ORDER BY `blog_date` ASC LIMIT ? OFFSET ? ");
        $stmt->bind_param("iii", $catid, $blogs_per_page, $skip);
        $stmt->execute();
        $result = $stmt->get_result();
    } else {
        $stmt = $conn->prepare("SELECT * FROM `blogs` INNER JOIN `blog_categories` ON `blogs`.`blog_categories_bc_id` = `blog_categories`.`bc_id` ORDER BY `blog_date` ASC LIMIT ? OFFSET ? ");
        $stmt->bind_param("ii", $blogs_per_page, $skip);
        $stmt->execute();
        $result = $stmt->get_result();
    }
...
?>
    ...
    <div class="th-pagination ">
        <ul>
            <li onclick="blogPagination(<?php echo $before; ?>);"><a class="hand"><i class="fa-regular fa-arrow-left"></i></a></li>
            <?php
            ...
            ?>
            <li onclick="blogPagination(<?php echo $after; ?>);"><a class="hand"><i class="fa-regular fa-arrow-right"></i></a></li>
        </ul>
    </div>
<?php
